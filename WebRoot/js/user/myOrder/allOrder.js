...
//		tr.push('<img src="../../../goodsimg/pms_1555075954.81667747.jpg" alt="">');
		...
		tr.push('<a href="">订单详情</a>');
		tr.push('<a class ="btn btn-small btn-line-gray pay" oImg = "'+ item.goodsImg +'" oName = "'+ item.productName +'" oid = "'+ item.goodsid +'" >晒单评价</a>');
		tr.push('<a href="">申请售后</a>');
		tr.push('</td>');
		tr.push('</tr>');
		tr.push('</tbody>');
		tr.push('</table>');
...
TAL.prototype.bindEvent = function () {
	...
	$(".order-actions").on("click",".pay",function(){
		// 获取商品ID
		util.cookie.set("oid",$(this).attr("oid")); 
		// 获取商品名称
		util.cookie.set("oName",$(this).attr("oName"));
		// 获取商品图片
		util.cookie.set("oImg",$(this).attr("oImg"))
		var next = "productReview.jsp"
		parent.location.assign(encodeURI(next));
		// 获取商品图片
